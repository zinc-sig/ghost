...
func init() {
	...
	superviseCmd.Flags().Int64Var(&superviseMaxMemoryBytes, "max-memory-bytes", 0, "Memory budget in bytes for the command and every process it starts, including one that left its group or session; they are killed past it and the trailer flags the kill, and any of them still running when the command exits is killed (0 = no enforcement)")
